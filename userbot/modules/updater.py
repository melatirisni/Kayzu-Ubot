...
async def update(xx, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await edit_or_reply(
        xx, "`ᴋᴀʏᴢᴜ-ᴜʙᴏᴛ Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
    )
...
async def upstream(event):
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy ᴋᴀʏᴢᴜ-ᴜʙᴏᴛ Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**⚡ ᴋᴀʏᴢᴜ-ᴜʙᴏᴛ Sudah Versi Terbaru**")
        return repo.__del__()
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            "**Ketik** `.update deploy` **Untuk Mengupdate ᴋᴀʏᴢᴜ-ᴜʙᴏᴛ.**"
        )
